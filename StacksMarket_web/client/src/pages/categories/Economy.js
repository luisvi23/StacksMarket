...
const Economy = () => {
  ...
  const subCategories = ['All', 'Trade War', 'Fed Rates', 'Inflation', 'Taxes', 'Macro Indicators', 'Treasuries'];

  const { data: pollsData, isLoading, error } = useQuery(
    ['polls', 'Economy', selectedSubCategory, searchTerm, currentPage],
    async () => {
      const params = new URLSearchParams({ category: 'Economy', page: currentPage, limit: 12 });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Economy</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400">Macro indicators and financial markets</p>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <form onSubmit={handleSearch} className="relative">
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search economy polls..." className="input pl-10 pr-4 w-full" />
