;; -----------------------------------------------------------
;; market-factory-v10-bias (Fixed-b LMSR + bias/virtual liquidity)
;;
;; Adds: buy-by-budget quotes (1 read-only call):
;;   - quote-buy-yes-by-budget(m, budget)
;;   - quote-buy-no-by-budget(m, budget)
;;
;; Also fixes: quote-buy-yes/no amount=0 => total=0 (needed for budget search)
;;
;; Replaces virtual shares with bias (virtual liquidity) that ONLY affects pricing.
;; Redemption/solvency remain based ONLY on real shares.
;;
;; Normalized biased cost:
;;   C_tilde(q) = b*ln(exp((qY+rY)/b)+exp((qN+rN)/b)) - b*ln(exp(rY/b)+exp(rN/b))
;;
;; Pricing uses qY,qN (real) and rY,rN (bias), but redemption uses ONLY real shares.
;; -----------------------------------------------------------
...
;; Helper: quote vacio (para amount=0)
(define-private (zero-buy-quote)
  { cost: u0, feeProtocol: u0, feeLP: u0, total: u0, drip: u0, brc20: u0, team: u0 }
)

;; Quote puro usando estado ya cargado (NO hace map-get? ni var-get)
(define-private (quote-buy-from-state
  (b uint) (qy uint) (qn uint) (rY uint) (rN uint)
  (amount uint) (yes? bool)
  (pB uint) (lB uint) (pctD uint) (pctB uint)
)
  (if (is-eq amount u0)
      (zero-buy-quote)
      (let (
        ...
        { cost: c, feeProtocol: fP, feeLP: fL, total: tot, drip: dr, brc20: br, team: tm }
      )
  )
)


;; FIX: amount=0 => total=0 (antes devolvia minimo u1)
(define-read-only (quote-buy-yes (m uint) (amount uint))
  (let ((b (get-b-or0 m)))
    (if (or (is-eq amount u0) (is-eq b u0))
        (ok (zero-buy-quote))
        (let (
          (qy (get-qy-or0 m))
          (qn (get-qn-or0 m))
...
;; ----------------------- BUY by budget (NO recursion) -----------------------
(define-constant BS-STEPS
  (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11 u12 u13 u14 u15 u16 u17 u18 u19 u20 u21 u22 u23)
)

(define-private (get-ys-or0 (m uint))
  (default-to u0 (get s (map-get? yes-supply { m: m })))
)

(define-private (get-ns-or0 (m uint))
  (default-to u0 (get s (map-get? no-supply { m: m })))
)

(define-private (is-market-tradable (m uint))
  (and (is-eq (get-status-str m) "open")
       (is-eq (get-paused m) false)
       (is-eq (get-initialized-bool m) true))
)

;; State tuple for fold-based binary search
;; { m, budget, pool, ys, ns, lo, hi }
;; State para el fold:
;; { budget, pool, ys, ns, b, qy, qn, rY, rN, pB, lB, pctD, pctB, lo, hi }

(define-private (bs-step-yes (dummy uint)
  (st { budget: uint, pool: uint, ys: uint, ns: uint
      , b: uint, qy: uint, qn: uint, rY: uint, rN: uint
      , pB: uint, lB: uint, pctD: uint, pctB: uint
      , lo: uint, hi: uint }))
  (let (
    (budget (get budget st))
    (pool   (get pool st))
    (ys     (get ys st))
    (ns     (get ns st))
    (b      (get b st))
    ...
    (if (>= lo hi)
        st
        (let (
          (mid (/ (+ lo (+ hi u1)) u2)) ;; ceil midpoint
          (q (quote-buy-from-state b qy qn rY rN mid true pB lB pctD pctB))
          (cost (get cost q))
          (tot  (get total q))
          (pool1 (+ pool cost))
          (ys1 (+ ys mid))
        )
          (if (and (<= tot budget) (>= pool1 ys1) (>= pool1 ns))
              { budget: budget, pool: pool, ys: ys, ns: ns
              , b: b, qy: qy, qn: qn, rY: rY, rN: rN
              , pB: pB, lB: lB, pctD: pctD, pctB: pctB
              , lo: mid, hi: hi }
              (let ((newHi (if (is-eq mid u0) lo (- mid u1))))
                { budget: budget, pool: pool, ys: ys, ns: ns
                , b: b, qy: qy, qn: qn, rY: rY, rN: rN
                , pB: pB, lB: lB, pctD: pctD, pctB: pctB
                , lo: lo, hi: newHi }
...
(define-private (bs-step-no (dummy uint)
  (st { budget: uint, pool: uint, ys: uint, ns: uint
      , b: uint, qy: uint, qn: uint, rY: uint, rN: uint
      , pB: uint, lB: uint, pctD: uint, pctB: uint
      , lo: uint, hi: uint }))
  (let (
    ...
    (if (>= lo hi)
        ...
        (let (
          (mid (/ (+ lo (+ hi u1)) u2))
          (q (quote-buy-from-state b qy qn rY rN mid false pB lB pctD pctB))
          (cost (get cost q))
          (tot  (get total q))
          (pool1 (+ pool cost))
          (ns1 (+ ns mid))
        )
          (if (and (<= tot budget) (>= pool1 ys) (>= pool1 ns1))
              { budget: budget, pool: pool, ys: ys, ns: ns
              , b: b, qy: qy, qn: qn, rY: rY, rN: rN
              , pB: pB, lB: lB, pctD: pctD, pctB: pctB
              , lo: mid, hi: hi }
...
(define-private (bs-buy-yes-fold (st0 { budget: uint, pool: uint, ys: uint, ns: uint
                                     , b: uint, qy: uint, qn: uint, rY: uint, rN: uint
                                     , pB: uint, lB: uint, pctD: uint, pctB: uint
                                     , lo: uint, hi: uint }))
  (let ((stF (fold bs-step-yes BS-STEPS st0)))
    (get lo stF)
  )
)

(define-private (bs-buy-no-fold (st0 { budget: uint, pool: uint, ys: uint, ns: uint
                                    , b: uint, qy: uint, qn: uint, rY: uint, rN: uint
                                    , pB: uint, lB: uint, pctD: uint, pctB: uint
                                    , lo: uint, hi: uint }))
  (let ((stF (fold bs-step-no BS-STEPS st0)))
    (get lo stF)
  )
)


(define-read-only (quote-buy-yes-by-budget (m uint) (budget uint))
  (if (or (is-eq budget u0) (not (is-market-tradable m)) (is-eq (get-b-or0 m) u0))
      (ok { shares: u0, cost: u0, feeProtocol: u0, feeLP: u0, total: u0, drip: u0, brc20: u0, team: u0 })
      (let (
        (pool (get-pool-or0 m))
        (ys   (get-ys-or0 m))
        (ns   (get-ns-or0 m))

        ;;  evita leer m-q-yes/m-q-no: son redundantes con supply
        (qy ys)
        (qn ns)
        ...
        (pB  (var-get protocol-fee-bps))
        (lB  (var-get lp-fee-bps))
        (pctD (var-get pct-drip))
        (pctB (var-get pct-brc))

        (mt  (get-max-trade-or0 m))
        (sum (+ pool budget))
        (hi0 (if (>= sum ys) (- sum ys) u0))
        (hi  (if (and (> mt u0) (< mt hi0)) mt hi0))
      )
        (if (is-eq hi u0)
            (ok { shares: u0, cost: u0, feeProtocol: u0, feeLP: u0, total: u0, drip: u0, brc20: u0, team: u0 })
            (let (
              (st0 { budget: budget, pool: pool, ys: ys, ns: ns
                   , b: b, qy: qy, qn: qn, rY: rY, rN: rN
                   , pB: pB, lB: lB, pctD: pctD, pctB: pctB
                   , lo: u0, hi: hi })
              (shares (bs-buy-yes-fold st0))
              (q (quote-buy-from-state b qy qn rY rN shares true pB lB pctD pctB))
            )
              (ok { shares: shares
                  , cost: (get cost q)
                  , feeProtocol: (get feeProtocol q)
                  , feeLP: (get feeLP q)
                  , total: (get total q)
                  , drip: (get drip q)
                  , brc20: (get brc20 q)
...
(define-read-only (quote-buy-no-by-budget (m uint) (budget uint))
  (if (or (is-eq budget u0) (not (is-market-tradable m)) (is-eq (get-b-or0 m) u0))
      (ok { shares: u0, cost: u0, feeProtocol: u0, feeLP: u0, total: u0, drip: u0, brc20: u0, team: u0 })
      (let (
        (pool (get-pool-or0 m))
        ...
        (if (is-eq hi u0)
            ...
            (let (
              (st0 { budget: budget, pool: pool, ys: ys, ns: ns
                   ...
                   , pB: pB, lB: lB, pctD: pctD, pctB: pctB
                   , lo: u0, hi: hi })
              (shares (bs-buy-no-fold st0))
              (q (quote-buy-from-state b qy qn rY rN shares false pB lB pctD pctB))
            )
              (ok { shares: shares
                  , cost: (get cost q)
                  , feeProtocol: (get feeProtocol q)
