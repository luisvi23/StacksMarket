;; ===========================================================
;; market-factory-v20-2-bias (Fixed-b LMSR + bias/virtual liquidity)
;;
...
(define-constant ERR-DEPRECATED-CLOSE-HEIGHT (err u786))
...
;; Deprecated in v19.4+: heights are not reliable for wall-clock market closes.
(define-public (set-market-close-height (m uint) (close-height uint))
  ERR-DEPRECATED-CLOSE-HEIGHT
)
...
(define-public (set-user-cap (m uint) (who principal) (cap uint))
  (begin
    (try! (only-admin))
    (asserts! (is-eq (get-initialized-bool m) true) ERR-NOT-INIT)
    ;; legacy setter: caps are no longer enforced in buy paths.
...
;; Deprecated alias: returns close unix timestamp, not a block height.
(define-read-only (get-close-height (m uint)) (get-close-time-or0 m))
