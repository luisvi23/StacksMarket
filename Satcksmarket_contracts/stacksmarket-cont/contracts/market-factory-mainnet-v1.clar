;; ------------------------------------------------------------
;; market-factory-v6-mainnet-sbtc (Fixed-b LMSR + fixed-per-share redemption + SELL)
...
;; - External dependency: sBTC mainnet token contract
...
;; Official sBTC mainnet contract principal
...
(define-constant ERR-TOKEN-READ    (err u761))  ;; could not read token balance (sbtc-token get-balance)
...
(define-private (ensure-user-balance (need uint))
  (let (
    (bal (unwrap! (contract-call? SBTC get-balance tx-sender) ERR-TOKEN-READ))
  )
...
(define-public (create-market (m uint) (initial-liquidity uint))
  (begin
    ...
    ;; Capture this contract principal once.
    (as-contract (var-set SELF tx-sender))

    ;; ADMIN -> CONTRACT (sBTC)
    (try! (contract-call? SBTC transfer initial-liquidity tx-sender (var-get SELF) none))
...
(define-public (add-liquidity (m uint) (amount uint))
  (begin
    ...
    ;; ADMIN -> CONTRACT (sBTC)
    (try! (contract-call? SBTC transfer amount tx-sender (var-get SELF) none))
    ...
    (let (
      ...
      (let (
        ...
        (let (
          ...
          ;; USER -> CONTRACT (base only) in sBTC
          (try! (contract-call? SBTC transfer base tx-sender (var-get SELF) none))

          ;; protocol fees: USER -> recipients
          (if (> feeP u0)
            (begin
              (if (> drip u0) (try! (contract-call? SBTC transfer drip tx-sender (var-get DRIP_VAULT) none)) true)
              (if (> brc  u0) (try! (contract-call? SBTC transfer brc  tx-sender (var-get BRC20_VAULT) none)) true)
              (if (> team u0) (try! (contract-call? SBTC transfer team tx-sender (var-get TEAM_WALLET) none)) true)
              true
            )
            true
          )
          ...
          ;; LP fee
          (if (> feeL u0)
            (try! (contract-call? SBTC transfer feeL tx-sender (var-get LP_WALLET) none))
            true
          )
            ...
            (let (
              ...
              (if (> totalFees u0)
                (begin
                  ...
                  (if (> feeP u0)
                    (begin
                      (if (> drip u0) (try! (contract-call? SBTC transfer drip user (var-get DRIP_VAULT) none)) true)
                      (if (> brc  u0) (try! (contract-call? SBTC transfer brc  user (var-get BRC20_VAULT) none)) true)
                      (if (> team u0) (try! (contract-call? SBTC transfer team user (var-get TEAM_WALLET) none)) true)
                      true
                    )
                    true
                  )
                  ...
                  ;; LP fee
                  (if (> feeL u0)
                    (try! (contract-call? SBTC transfer feeL user (var-get LP_WALLET) none))
                    true
                  )
                )
                true
              ...
              ;; pool -> USER (base) (contract sends its own sBTC)
              (as-contract
                (try! (contract-call? SBTC transfer base (var-get SELF) user none)))
...
(define-public (sell-yes-auto (m uint) (amount uint) (min-proceeds uint)) (do-sell-auto m amount true min-proceeds))
(define-public (sell-no-auto  (m uint) (amount uint) (min-proceeds uint)) (do-sell-auto m amount false min-proceeds))
...
(define-private (redeem-yes (m uint))
  (let (
    ...
    (as-contract (try! (contract-call? SBTC transfer payout (var-get SELF) rcpt none)))
...
(define-public (withdraw-surplus (m uint))
  (let (
    ...
    (begin
      ...
      (as-contract (try! (contract-call? SBTC transfer p (var-get SELF) ADMIN none)))
