...
(define-map m-close-height { m: uint } { v: uint }) ;; 0 = no auto-close, else trading closes at this block height
...
(define-map default-caps { m: uint } { cap: uint })
...
(define-constant ERR-MARKET-CLOSED-BY-HEIGHT (err u784))
...
(define-private (get-close-height-or0 (m uint))
  (default-to u0 (get v (map-get? m-close-height { m: m })))
)
...
(define-private (get-default-cap-or0 (m uint))
  (default-to u0 (get cap (map-get? default-caps { m: m })))
)

(define-private (get-effective-cap-or0 (m uint) (who principal))
  (match (map-get? user-caps { m: m, user: who })
    row (get cap row)
    (get-default-cap-or0 m))
)
...
(define-read-only (get-cap-effective (m uint) (who principal))
  (get-effective-cap-or0 m who)
)

(define-read-only (get-default-cap (m uint))
  (get-default-cap-or0 m)
)
...
(define-private (ensure-open (m uint))
  (begin
    ...
    (let ((closeH (get-close-height-or0 m)))
      (if (> closeH u0)
          (asserts! (< block-height closeH) ERR-MARKET-CLOSED-BY-HEIGHT)
          true))
    (ok true)
  )
)
...
(define-public (set-market-close-height (m uint) (close-height uint))
  (begin
    (try! (only-admin))
    (asserts! (is-eq (get-initialized-bool m) true) ERR-NOT-INIT)
    ;; allow u0 to clear; otherwise require a future block
    (if (is-eq close-height u0)
        true
        (asserts! (> close-height block-height) (err u785)))
    (map-set m-close-height { m: m } { v: close-height })
    (ok true)
  )
)
...
(define-public (set-default-cap (m uint) (cap uint))
  (begin
    (try! (only-admin))
    (asserts! (is-eq (get-initialized-bool m) true) ERR-NOT-INIT)
    (map-set default-caps { m: m } { cap: cap })
    (ok true)
  )
)
...
(define-public (create-market (m uint) (initial-liquidity uint))
  (begin
    ...
    (let ((liq-stx (ustx->stx initial-liquidity)))
      ...
      (map-set m-close-height { m: m } { v: u0 })
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        (let (
          (cap   (get-effective-cap-or0 m tx-sender))
...
(define-public (buy-yes-auto (m uint) (amount uint) (target-cap uint) (max-cost uint))
  (begin
    ...
    ;; target-cap kept only for ABI/UI compatibility; user caps are admin-managed.
    target-cap
...
(define-read-only (get-market-snapshot (m uint))
  (let (
    ...
    (closeHeight (get-close-height-or0 m))
    (tradingOpenNow
      (let ((closeH (get-close-height-or0 m)))
        (and (is-eq status "open")
             (is-eq paused false)
             (if (> closeH u0) (< block-height closeH) true))))
...
(define-read-only (get-close-height (m uint)) (get-close-height-or0 m))
(define-read-only (is-trading-open-now (m uint))
  (let (
    (status (get-status-str m))
    (paused (get-paused m))
    (closeH (get-close-height-or0 m))
  )
    (and (is-eq status "open")
         (is-eq paused false)
