...
;; p-yes is integer percent. Contract derives bias difference:
;;   rDiff = b * ln( p / (1-p) )
;;
;; Implementation stores unsigned rY/rN:
;;   if p>=50: rY=rDiff, rN=0  else: rY=0, rN=rDiff
;;
;; This preserves initial P(YES)=exp(rY/b)/(exp(rY/b)+exp(rN/b)) exactly.
(define-public (set-market-bias (m uint) (p-yes uint))
  (begin
    ...
    ;; clamp 1..99 (recommended)
    (asserts! (and (>= p-yes u1) (<= p-yes u99)) ERR-BIAS-PCT)

    ;; only allowed before any real shares exist
    (let (
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        ;; compute proceeds using biased curve on REAL q
        (let (
          ...
          (let (
            ...
            ;; fees paid by user wallet (same as v7)
            (if (> totalFees u0)
              (begin
...
;; AUTO SELL with slippage on net proceeds
...
(define-public (resolve (m uint) (result (string-ascii 3)))
  (begin
    ...
    ;; solvency check uses REAL supply only (bias never redeems)
    (let (
      (ys (default-to u0 (get s (map-get? yes-supply { m: m })) ))
      (ns (default-to u0 (get s (map-get? no-supply  { m: m })) ))
      (p  (get-pool-or0 m))
