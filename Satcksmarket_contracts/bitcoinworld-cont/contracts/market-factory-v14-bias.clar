;; ===========================================================
;; market-factory-v16-bias (Fixed-b LMSR + bias/virtual liquidity)
;;
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        (feePStx (ceil-bps baseStx pB))
        (feeLStx (ceil-bps baseStx lB))
        (dripStx (/ (* feePStx (var-get pct-drip)) u100))
        (brcStx  (/ (* feePStx (var-get pct-brc))  u100))
        (teamStx (- feePStx (+ dripStx brcStx)))
        (totalStx (+ baseStx (+ feePStx feeLStx)))

        (baseUstx (stx->ustx baseStx))
        (feePUstx (stx->ustx feePStx))
        (feeLUstx (stx->ustx feeLStx))
        (dripUstx (stx->ustx dripStx))
        (brcUstx  (stx->ustx brcStx))
        (teamUstx (stx->ustx teamStx))
        (totalUstx (stx->ustx totalStx))
      )
        (try! (ensure-user-balance totalUstx))
        ...
        (let (
          ...
          (asserts! (<= (+ spent totalUstx) cap) (err u731))
          ...
          (try! (xfer-in totalUstx))

          ;;  fees paid by CONTRACT -> recipients
          (try! (xfer-out dripUstx (var-get DRIP_VAULT)))
          (try! (xfer-out brcUstx  (var-get BRC20_VAULT)))
          (try! (xfer-out teamUstx (var-get TEAM_WALLET)))
          (try! (xfer-out feeLUstx (var-get LP_WALLET)))
          ...
          (add-spent m tx-sender totalUstx)
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        (asserts! (<= totalUstx max-cost) ERR-SLIPPAGE)
        (try! (ensure-user-balance totalUstx))
      ...
      (let (
        ...
        (let (
          ...
          (let (
            ...
            (feeP0Stx (ceil-bps baseStx pB))
            (feePStx  (minu feeP0Stx baseStx))
            (remStx   (- baseStx feePStx))
            (feeL0Stx (ceil-bps baseStx lB))
            (feeLStx  (minu feeL0Stx remStx))

            (dripStx (/ (* feePStx (var-get pct-drip)) u100))
            (brcStx  (/ (* feePStx (var-get pct-brc))  u100))
            (teamStx (- feePStx (+ dripStx brcStx)))

            (totalFeesStx (+ feePStx feeLStx))
            (netStx (- baseStx totalFeesStx))
      ...
      (let (
        ...
        (let (
          ...
          (let (
            ...
            (asserts! (>= pool baseUstx) ERR-POOL-LIQUIDITY)
            (asserts! (>= netUstx min-proceeds) ERR-SLIPPAGE)
...
(define-read-only (quote-buy-yes (m uint) (amount uint))
  (let (
    ...
    (if true
        (ok { cost: cU, feeProtocol: fPU, feeLP: fLU, total: totU, drip: drU, brc20: brU, team: tmU })
        (err u0))
  )
)
...
;; budget is TOTAL (base + fees) in uSTX. We compute a conservative baseBudget in STX,
;; invert, refine, then return final quote for those shares. (1 read-only call in frontend)
(define-read-only (quote-buy-yes-by-sats (m uint) (budget uint))
  (let (
    ...
    (pB (var-get protocol-fee-bps))
    (lB (var-get lp-fee-bps))
    (budgetStx (ustx->stx budget))
  )
    (if (or (is-eq b u0) (is-eq budgetStx u0))
        (ok { shares: u0, budget: budget, baseBudget: u0, quote: EMPTY-BUY-QUOTE })
        (let (
          (baseBudgetStx (tighten-base-budget budgetStx pB lB))

          (qSelfP (+ qy rY))
          (qOtherP (+ qn rN))

          (s0 (invert-buy-shares b qSelfP qOtherP baseBudgetStx))
          (s  (refine-shares-updown b qy qn rY rN baseBudgetStx true s0))
        )
              ...
              (if (and (> s u0) (> tot budget))
                  (let ((s2 (- s u1)))
                    (match (quote-buy-yes m s2)
                      q2 (ok { shares: s2, budget: budget, baseBudget: (stx->ustx baseBudgetStx), quote: q2 })
                      e2 (ok { shares: s2, budget: budget, baseBudget: (stx->ustx baseBudgetStx), quote: EMPTY-BUY-QUOTE })
                    )
                  )
                  (ok { shares: s, budget: budget, baseBudget: (stx->ustx baseBudgetStx), quote: q })
              )
            )
            e
            (ok { shares: u0, budget: budget, baseBudget: (stx->ustx baseBudgetStx), quote: EMPTY-BUY-QUOTE })
          )
        )
    )
  )
  ...
  (let (
    ...
    (if (or (is-eq b u0) (is-eq budgetStx u0))
        ...
        (let (
          ...
          (s0 (invert-buy-shares b qSelfP qOtherP baseBudgetStx))
          (s  (refine-shares-updown b qy qn rY rN baseBudgetStx false s0))
        )
